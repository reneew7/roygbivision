...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for (int i = 0; i < an; i++) {
        // track: seen[m[i]'s b index] == 0 if it is unseen because of calloc
        // set to a different number if seen
        if (seen[m[i].bi] == 0) {
            // move good match to front of m
            m[count] = m[i];
            count++;
            seen[m[i].bi] = 1;
        }
...
point project_point(matrix H, point p)
{
    ...
    // x bar = squiggly x / squiggly w
    q.x = (float) proj.data[0][0] / proj.data[2][0];
    q.y = (float) proj.data[1][0] / proj.data[2][0];
    free_matrix(proj);
    free_matrix(c);
    return q;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++) {
        proj = project_point(H, m[i].p);
        dist = point_distance(proj, m[i].q);
        if (dist < thresh) {
            count++;
        } else { 
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        double xp = matches[i].q.x;  // projected point
        double y  = matches[i].p.y;
        double yp = matches[i].q.y;
        // TODO: fill in the matrices M and b.
        /*
        M: mx1 my1 1 0... dx
        M: 0 0 0 mx1 my1 1 dy
        */
        M.data[i * 2][0] = x;
        M.data[i * 2][1] = y;
        M.data[i * 2][2] = 1;
    ...
    H.data[2][2] = 1;  // last one pad with 1?
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; i++) {
        ...
        // 8 DoF / 2 = 4 matches
        matrix nextHomography = compute_homography(m, 4);
        e = model_inliers(nextHomography, m, n, thresh);
        // want max number of inliers
        free_matrix(nextHomography);
        if (e > best) {
            // free_matrix(Hb);
...
image combine_images(image a, image b, matrix H)
{
    ...
    float val;
    for (int i = 0; i < c.c; i++) {
        for (int j = topleft.y; j < h; j++) {
            for (int k = topleft.x; k < w; k++) {
                p = make_point((float) (k + dx), (float) (j + dy));
                proj = project_point(H, p);
                if (proj.x < b.w && proj.y < b.h && proj.x >= 0.0 && proj.y >= 0.0) {
                    val = bilinear_interpolate(b, proj.x, proj.y, i);
                    set_pixel(c, k, j, i, val);
                }
            }
        }
    }
